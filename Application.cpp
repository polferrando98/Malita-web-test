...
Application::Application()
{
	...
	list_modules.push_back(transition);
	...
	list_modules.push_back(lights);
	list_modules.push_back(scene);
	list_modules.push_back(pathfinding);
...
void Application::FinishUpdate()
{
	...
	if (fps_timer.Read() >= 1000) {
		avg_fps = current_frames;
		current_frames = 0;
		fps_timer.Start();
	}

	float capped_duration = 1000 / frame_rate_cap;
	//SDL.Delay is imprecise, this will keep the program busy until the frame slows down, only needed if frame_rate_cap < VSYNC
	//while (is_game && ms_timer.Read() < capped_duration)
	//	continue;
}
